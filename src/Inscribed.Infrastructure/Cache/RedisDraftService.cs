...
public sealed class RedisDraftService : IDraftService
{
    ...
    public async Task<IReadOnlyList<DraftBlock>?> GetDraftAsync(string clientId, string? locale, string userId, string slug, CancellationToken cancellationToken = default)
    {
        var key = BuildKey(clientId, locale, userId, slug);
