...
{
    public static IEndpointRouteBuilder MapInscribedIssuerEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/google/callback", async (string? state, string? code, string? error, HttpContext context, IGoogleLoginService login, IOptions<AuthIssuerOptions> options, CancellationToken ct) =>
        {
        ...
        group.MapPost("/refresh", async (HttpContext context, IRefreshTokenService refresh, IOptions<AuthIssuerOptions> options, CancellationToken ct) =>
        {
        ...
        group.MapPost("/logout", async (HttpContext context, IRefreshTokenService refresh, IOptions<AuthIssuerOptions> options, CancellationToken ct) =>
        {
            var rawToken = context.Request.Cookies[options.Value.Cookie.Name];
            if (!string.IsNullOrWhiteSpace(rawToken))
            {
    ...
    private static void AppendRefreshCookie(HttpContext context, AuthIssuerOptions options, string token, DateTime expiresAtUtc)
    {
    ...
    private static void DeleteRefreshCookie(HttpContext context, AuthIssuerOptions options)
    {
