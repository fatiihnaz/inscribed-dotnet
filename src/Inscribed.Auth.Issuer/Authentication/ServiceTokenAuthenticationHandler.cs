...
namespace Inscribed.Auth.Issuer.Authentication;

public static class InscribedIssuerSchemes
{
...
internal sealed class ServiceTokenSchemeSelector : IAuthenticationSchemeSelector
{
    public string? Select(HttpRequest request)
        => ServiceTokenLocator.Locate(request) is null ? null : InscribedIssuerSchemes.ServiceToken;
}
