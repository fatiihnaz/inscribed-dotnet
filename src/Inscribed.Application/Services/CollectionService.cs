...
public sealed class CollectionService : ICollectionService
{
    ...
    public async Task<CollectionItemResponse> UpsertAsync(string key, string slug, UpsertCollectionItemRequest request, ClaimsPrincipal user, string updatedBy, CancellationToken cancellationToken = default)
    {
        var normalizedSlug = SlugNormalizer.NormalizeBlockPath(slug);
        var policy = _policyResolver.Resolve(key);
        key = policy.Key;
    ...
    public async Task<CollectionItemResponse> CreateAutoSlugAsync(string key, CreateCollectionItemRequest request, ClaimsPrincipal user, string updatedBy, CancellationToken cancellationToken = default)
    {
        var policy = _policyResolver.Resolve(key);
        key = policy.Key;
    ...
    public async Task SaveNewDraftAsync(string key, string userId, ClaimsPrincipal user, SaveNewDraftRequest request, CancellationToken cancellationToken = default)
    {
        var policy = _policyResolver.Resolve(key);
        key = policy.Key;
