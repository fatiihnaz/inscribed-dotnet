...
| `displayName` | no | what the collection itself is called for humans, carried on `/schema` and `/cms/collections/me`; omitting it names the collection by its `key` |
...
- `displayName`, if present, is non-blank and at most 80 characters
