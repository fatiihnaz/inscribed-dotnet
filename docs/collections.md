...
| `sortable` | no | enables `?sort=` on this field; only `ShortText`, `Number` and `Date` may be sortable |
...
200 → { "collectionKey": "news", "slug": "eski-haber", "version": 1 }
```
