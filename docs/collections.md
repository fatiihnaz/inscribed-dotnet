...
// collections/projects.json
{
  ...
  "fields": [
    ...
    { "name": "repo",  "type": "Text", "help": "owner/name on GitHub" },
...
`slug.from` must reference a `Text` or `ShortText` field: slugs derive from short human-readable titles, and deriving one from rich text, arrays, or numbers has no sensible normalization. `slug.from` is only valid with `AutoGenerated`.
