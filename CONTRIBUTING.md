...
- **Migrations apply automatically at startup** for both contexts by default (`Database:MigrateOnStartup`, default `true`). You never run `dotnet ef database update` in normal development; starting the API is the update. This also means starting the API against a shared database applies your half-finished migration to it. Set the flag `false` for multi-instance deploys (EF's `Migrate()` is not safe to run concurrently from several instances) and apply migrations as a one-shot instead: `docker compose --profile migrate run --rm api-migrate`, which runs the image with `RUN_MIGRATIONS_AND_EXIT=true`. With the flag `false`, an instance whose schema is behind refuses to start rather than serving against a stale schema.
