...
Listing supports `offset`/`limit` paging (limit clamped to 100), `sort` over `slug`, `createdAt`, `updatedAt` or any field declared `sortable` (`?sort=publishedAt:desc`, because row age is not editorial order), and equality filters on `filterable` fields via plain query parameters, e.g. `GET /cms/collections/News/?featured=true&tags=release`. Filters stay **exact matches**; text search is `?q=`, which matches every word of the query in the `displayField` or the slug regardless of case and diacritics (`isik` finds "Işık"), ranks the closest titles first, and falls back to similar titles flagged `"approximate": true` when nothing matches (see [docs/collections.md](docs/collections.md#searching)). Deleting an item **archives** it: `DELETE …/{slug}?version=` hides it from every read and answers with the item's version, which archiving deliberately does not consume: the same number archives, restores and still publishes afterwards, because the content never changed. `POST …/{slug}/restore?version=` brings it back, and `?archived=true` lists the archive for editors only. An archived slug stays reserved, so a restore can never collide, and every write aimed at an archived item is refused with **409** carrying `"reason": "archived"` instead of a version conflict that would send an editor to a merge screen. `GET /cms/collections/me` tells a panel which collections the current user may create in, with their schemas and an `itemCount` each, so the editor UI is fully schema-driven and can badge a sidebar without opening every collection.
...
| Method & path | Policy | Purpose |
|---|---|---|
| `GET /cms/collections/me` | ContentWrite | collections the caller may create in, with schemas and item counts |
| `GET /cms/collections/{key}/schema` | anon\* | field schema of a collection |
| `GET /cms/collections/{key}/?offset=&limit=&locale=&sort=&archived=&q=&field=` | anon\* | paged, sortable, filterable, searchable listing (`archived=true` is editor-only) |
| `GET /cms/collections/{key}/lookup?q=&slugs=&locale=&limit=` | anon\* | slug + label pairs for reference pickers: `q` searches the `displayField` and slug, `slugs` resolves chosen ones |
