...
- **Schema-validated collections.** Each collection is a code policy: field schema, slug strategy (user-defined or auto-generated from a field), per-user edit rules, optional enrichment, optional anonymous reads. Payloads are validated and unknown fields rejected.
...
Collections hold structured items that are not blocks on a page. Each collection is defined in code by an `ICollectionPolicy` (see [Architecture](#architecture)) that declares:

- a **schema** of typed fields (`Text`, `RichText`, `Bool`, `Url`, `StringArray`, `Date`, `Number`, `ObjectArray`, `ShortText`, `LongText`), each optionally `Required`, `Filterable`, `ReadOnly`, or carrying nested `ItemFields` for `ObjectArray`;
- a **slug source**: `UserDefined` (client supplies the slug, `PUT` creates) or `AutoGenerated` (slug derived from a field such as the title, `POST` creates, collisions get `-2`, `-3`, … suffixes);
- **permissions**: `CanCreate`/`CanEdit` per user, and `AllowAnonymousRead` for public listings;
- an optional **enrichment** hook that can augment item data on read.
...
|---|---|---|---|
| Collection definition | `ICollectionPolicy` ([src](src/Inscribed.Application/Contracts/Policies/ICollectionPolicy.cs)) | `NewsCollectionPolicy` | schema, slug strategy, permissions, enrichment per collection |
